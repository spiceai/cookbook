...
Startup logs vary by runtime version. Continue when `taxi_trips` is registered and the runtime is ready.
...
Restart logs vary by version and environment. Continue when `taxi_trips` is registered with cayenne acceleration enabled.
