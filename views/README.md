...
Startup logs vary by version and environment. Continue once the runtime reports readiness.
...
Output from `spice add spiceai/tpch` varies by version and environment. Continue when the TPCH datasets are registered and initial refreshes complete.
...
Refresh logs vary by version and environment. Continue once the `supplier_order_waits` view is registered and initial refresh completes.
...
Scheduled refresh logs vary by version and environment. Continue when you observe periodic refresh activity for `supplier_order_waits`.
