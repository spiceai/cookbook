...
Startup logs vary by version and environment. Continue once `time_series` and `local_time_series` are registered and the runtime is ready.
...
The refresh log details (timestamps, duration, and byte counts) vary by environment. Continue once both `time_series` and `local_time_series` refreshes complete.
